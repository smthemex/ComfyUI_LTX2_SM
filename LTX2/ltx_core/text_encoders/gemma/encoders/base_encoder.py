...
#         )
#         self.feature_extractor_linear.to(normed_concated_encoded_text_features.device)
#         output=self.feature_extractor_linear(normed_concated_encoded_text_features.to(encoded_text_features_dtype))
#         self.feature_extractor_linear.to("cpu")
...
#         input_ids = torch.tensor([[t[0] for t in token_pairs]], device=inf_device)
#         attention_mask = torch.tensor([[w[1] for w in token_pairs]], device=inf_device)
#         if self.model.device==torch.device("cpu"):
#             self.model.to(inf_device)
#         #print(f"Model config hidden_size: {self.model.config.text_config.hidden_size}") #3840
#         #print(input_ids.shape,attention_mask.shape) #torch.Size([1, 1024]) torch.Size([1, 1024])
#         outputs = self.model(input_ids=input_ids,pixel_values=None, attention_mask=attention_mask, output_hidden_states=True)
...
# def module_ops_from_gemma_root(gemma_root: str,clip_path: str) -> tuple[ModuleOps, ...]:
...
#     #     return module
#     def load_gemma(module: GemmaTextEncoderModelBase) -> GemmaTextEncoderModelBase:
#         from transformers import Gemma3ForConditionalGeneration, Gemma3Config,Gemma3Model
