...
from ..encoders.base_encoder import GemmaTextEncoderModelBase
...
class AVGemmaTextEncoderModel(GemmaTextEncoderModelBase):
    ...
    def _run_connectors(
        ...
        #print(encoded_input.device,connector_attention_mask.device)
        if self.embeddings_connector.device != encoded_input.device:
            self.embeddings_connector.to(encoded_input.device)
        encoded, encoded_connector_attention_mask = self.embeddings_connector(
            encoded_input,
            connector_attention_mask,
        )
        self.embeddings_connector.to("cpu")
        ...
        if self.audio_embeddings_connector.device != encoded_input.device: 
            self.audio_embeddings_connector.to(encoded_input.device)
        encoded_for_audio, _ = self.audio_embeddings_connector(encoded_input, connector_attention_mask)
        self.audio_embeddings_connector.to("cpu")
        torch.cuda.empty_cache()
